function findCaliforniaCafes(searchTerm) {
    ...
    const matchingCafes = cafes.filter(cafe => cafe.name.includes(searchTerm));
  
    // Map the matching cafes to their corresponding places
    const results = matchingCafes.map(matchingCafe => {
      const placeDetails = places.find(place => place.id === matchingCafe.place_id);
      return { ...matchingCafe, ...placeDetails };
    });
